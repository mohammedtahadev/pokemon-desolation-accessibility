...
class Game_Player < Game_Character
  unless method_defined?(:access_mod_original_update)
  ...
  unless method_defined?(:access_mod_original_initialize)
...
def is_path_passable?(x, y, d)
    ...
    for event in $game_map.events.values
      if event.x == tele_x && event.y == tele_y && is_teleport_event?(event)
        return false
      end
    end
...
class Scene_Map
  ...
  unless method_defined?(:access_mod_original_main)
...
CUSTOM_NAMES_FILE = "pra-custom-names.txt" unless defined?(CUSTOM_NAMES_FILE)
...
class Game_Player < Game_Character
  unless method_defined?(:access_mod_walk_original_update)
